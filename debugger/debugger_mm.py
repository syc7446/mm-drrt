...
initial_conf0_6 = (-0.3187382163228052, -0.4015353053923896, -0.8963159896307984, 0.39277395, 0.33330058, 0.0, -1.52238431, 2.72170996, -1.21946936, -2.98914779)
initial_conf1_6 = (-0.5253967642784119, 0.9448966383934021, 0.7533274800902596, 0.39277395, 0.33330058, 0.0, -1.52238431, 2.72170996, -1.21946936, -2.98914779)
robots = [9, 10]
...
roadmaps, heuristic_vals, initial_conf, final_conf = [], [], [], []
file = open('data_mm', 'rb')
db = pickle.load(file)
for r in range(len(robots)):
    each_roadmaps = []
    each_heuristic_vals = []
    for p in range(4):
        each_roadmaps.append(db['roadmap'+str(r)+'_'+str(p + 6)])
        each_heuristic_vals.append(db['heuristic_vals'+str(r)+'_'+str(p + 6)])
